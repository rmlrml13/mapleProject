...
class UnitManager:
    ...
    def command(self, event: Event):
        ...
        # 이동 명령
            ...
            # 커서 애니메이션 추가
            self.game.effect_manager.add_effect(Effect(self.game,event.x,event.y,self.game.cursor_manager.command_circle_effect_blueprint))
    ...
    def game_object_select(self, event: Event):
        ...
        if self.game.cursor_manager.state is CursorManager.CursorManager.CURSOR_DRAG:
            ...
            if len(next_selected_game_object_list) is 0:
                for game_object in self.game_object_list:
        ...
        # 하나라도 선택 된게 있으면 바꿈
        # 그렇지 않으면 아무것도 하지 않음
        if len(next_selected_game_object_list) is not 0:
